...
public class BahriSpell4 : DamageSpell
{
    ...
    private void Spell_4_Missiles(){
        ...
        if(targets.Count > 0){
            foreach(GameObject target in targets){
                ...
                Spell4Trigger spell4Trigger = missile.GetComponent<Spell4Trigger>();
                spell4Trigger.SetBahriSpell4(this);
                spell4Trigger.SetTarget(target);
    ...
    */
    private IEnumerator Spell_4_Target(GameObject missile, GameObject target){
        ...
        while(missile){
            missile.transform.position = Vector3.MoveTowards(missile.transform.position, target.transform.position, spellData.speed * Time.deltaTime);
            yield return null;
        }
    }
